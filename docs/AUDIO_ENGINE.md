...
Physical iOS 18.4.1 testing showed that `getUserMedia` makes an already digitally active drone physically audible and that the same drone remains audible after capture stops. The microphone and drone retain separate contexts, and the microphone analysis graph never connects to destination; the shared change is WebKit's page audio-session/output route.

Reference activation now capability-detects `navigator.audioSession` without user-agent sniffing. For an inactive non-`playback` session it assigns `playback` before constructing output. A retained stopped output context is discarded, then its replacement graph and oscillator are created synchronously in the same gesture. The prior session type is restored on the next task only if the value is still `playback`, so the repair cannot overwrite a microphone or another owner that has changed the policy. An already active session is not mutated. Browsers without the draft API retain the existing lazy Web Audio path, and reference activation never calls `getUserMedia`.
