...
Harmonic tests verify exact A4/C4 fundamentals, integer partial multiples, range boundaries, coefficient normalization, the `0.16` worst-case peak ceiling, one-oscillator note transitions, fallback sine behavior, and cleanup. Audio-session tests cover ordered/bounded snapshots, supported, rejected, throwing, and unavailable setters, prior-type restoration, microphone Start/Stop ordering, and the absence of microphone samples.
