...
When `navigator.audioSession` exists and is inactive, production reference activation temporarily prepares `playback` before output-context construction. A retained stopped context is replaced after preparation; a fresh page simply creates its first context in the prepared session. The oscillator is started immediately in the same trusted gesture. The prior type is restored on the next task only while the temporary `playback` value is still present. Active sessions are left unchanged, browsers without the capability use the normal Web Audio path, and no microphone request is made. Web Audio remains the sole production backend; native media remains diagnostic only.
